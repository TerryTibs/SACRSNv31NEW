...
CONFIG = {
    ...
    "epochs": 300,
    "lr": 3e-4,             # Safe start
    "grad_clip": 0.5,       # [FIX] Tighter clipping
    "replay_size": 200,
...
print(f"--- SACRSN v64: BULLETPROOF EDITION ---")
...
class EMA_VQ(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Clamp distance to avoid negative sqrt NaN
...
class SACRSN_v64(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [FIX] Initialize weights carefully to prevent NaN explosion
        self.apply(self._init_weights)

    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            # Very conservative init for complex-valued stability
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Complex Tanh Bounding (Prevents Explosion)
            update = torch.complex(torch.tanh(update.real), torch.tanh(update.imag))
            
            cand_state = 0.6 * curr_state + 0.4 * update
            
            # [FIX] Safer Phase Calculation (Guard against NaN angles)
            safe_cand = cand_state + torch.complex(torch.tensor(1e-8, device=DEVICE), torch.tensor(1e-8, device=DEVICE))
            diff = torch.abs(safe_cand.angle() - prev_angle)
            diff = torch.min(diff, 2*math.pi - diff)
            accum['phase'] += active_mask.squeeze() * diff.mean(dim=-1)
            prev_angle = safe_cand.angle()
...
def main():
    ...
    model = SACRSN_v64(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
