...
CONFIG = {
    ...
    # The Shadow (Dynamic Gating)
    "shadow_weight_max": 0.3, # [MODIFIED] Max influence, scales with Uncertainty
    "critic_weight": 0.1,
    ...
    "entropy_weight_start": 0.10, # Start high (Exploration)
    "entropy_weight_end": 0.01,   # End low (Crystallization)
    ...
    # [NEW] Deadband Depth Targeting (Freedom within limits)
    "depth_min": 2.0,
    "depth_max": 6.0,
    "depth_penalty_weight": 0.05, 
}

print(f"--- SACRSN v66: THE CRYSTALLIZING EDITION ---")
...
class SACRSN_v66(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [UPGRADE] Shadow Gating: Shadow only influences if we are Uncertain
            w_unc = meta_values[:, 3:4]
            dynamic_shadow_weight = CONFIG["shadow_weight_max"] * w_unc.unsqueeze(-1)
...
def calculate_loss(logits, y, stats, meta_vals, plasticity=1.0, epoch=0, total_epochs=1):
    ...
    # [FIX] Deadband Depth Targeting (Freedom within limits)
    if isinstance(stats.get('depth_per_sample'), torch.Tensor):
        d = stats['depth_per_sample']
        # Penalize only if < min or > max
    ...
    # [FIX] Entropy Annealing (High -> Low)
...
def main():
    ...
    model = SACRSN_v66(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
