...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t: torch.Tensor, gw_state: torch.Tensor,
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # Improvement 6 — module specialisation loss.
            # Penalise pairwise cosine similarity between module proposals.
            # If modules produce identical outputs, GWT competition is
            # meaningless — the arbitrator has nothing to choose between.
            # Upper-triangle mask covers all 10 unique pairs (5 choose 2).
            with torch.no_grad():
                props_flat = torch.cat([proposals.real, proposals.imag], dim=-1)
                props_norm = F.normalize(props_flat, dim=-1)      # (B, 5, 2d)
            # Similarity matrix computed in-graph so grad flows to module params
            props_flat_g = torch.cat([proposals.real, proposals.imag], dim=-1)
            props_norm_g = F.normalize(props_flat_g, dim=-1)
            sim_matrix   = torch.bmm(props_norm_g,
                                     props_norm_g.transpose(1, 2))  # (B, 5, 5)
            pair_mask    = torch.triu(torch.ones(5, 5, device=DEVICE),
                                      diagonal=1).bool()
            # mean over pairs per sample, then mask to active only
            div_penalty  = sim_matrix[:, pair_mask].mean(dim=-1)   # (B,)
            accum['module_div'] += mask_vec * div_penalty
