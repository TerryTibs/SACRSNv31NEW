...
CONFIG = {
    ...
    "max_recursion":     8,     # Max cortical cycles per token
    ...
    "shadow_weight":     0.3,   # Influence of the losing modules' aggregate
    ...
    "gate_sparsity_weight": 0.01,  # Penalise diffuse gate distributions
    "slot_balance_weight":  0.01,  # Reward uniform memory-slot usage
    "phase_reg":            0.01,  # Penalise large inter-step phase jumps
    "sensory_reg":          0.01,  # Reward diverse sensory attention
...
class StableComplexNorm(nn.Module):
    ...
    Engineering: separate LayerNorm prevents magnitude collapse in either
    component; epsilon guard avoids division-by-zero in sparse states.
    """
...
class ComplexLayerNorm(nn.Module):
    """
    Phase-preserving normalisation: normalise magnitude, keep angle.
    ...
    Neurolinguistic: this is analogous to gain control in sensory cortex —
    the 'volume' of activation is normalised while its 'direction' (phase)
...
class HermitianGating(nn.Module):
    ...
    Engineering note: the Hermitian product's real part is always symmetric
    (positive definite for equal vectors), giving a natural self-similarity
    score that avoids the need for an explicit normalisation step.
    """
    def __init__(self, dim: int):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor):
        ...
        if self.training:
            with torch.no_grad():
                ...
                cluster_size = cluster_size + (cluster_size < 1.0).float()  # min denominator
...
class HebbianSensory(nn.Module):
    """
    Hebbian sensory module with visual (Real) and auditory (Imag) palettes.

    Neurolinguistic:
        NLP submodalities (Bandler & Grinder): internal representations
        possess sensory qualities — brightness, tone, spatial position.
        This module learns 'submodality anchors' that are queries over
        the current workspace state.

        Visual palette  ← queried by Real component of z ('what' stream)
        Auditory palette← queried by Imag component of z ('when/how' stream)

    Math:
        The two palette outputs are combined as a complex 90° rotation:
        experience = vis_c + i·aud_c  (Imaginary part is aud rotated by π/2)
        This preserves orthogonality between the two sensory channels.

    Engineering:
        Palettes are initialised with nn.init.orthogonal_ so that anchors
        span the embedding space uniformly at the start of training.
    """
    ...
    def forward(self, gw_state: torch.Tensor):
        ...
        # Combine: auditory is 90° phase-rotated (i·aud) to stay orthogonal
...
class CriticModule(nn.Module):
    """
    Adversarial/ethical shadow: rotates the workspace by 90° in ℂ.

    Math:
        critic(z) = i·W(z)   where i·(a+ib) = −b + ia
        This is a pure rotation — the critic can never amplify, only redirect.

    Neurolinguistic:
        Models the brain's 'devil's advocate' function — the left-hemisphere
        interpreter challenging right-hemisphere holistic proposals.
        Acts as an ethical / plausibility filter.
    """
    def __init__(self, dim: int):
        super().__init__()
    ...
    def forward(self, gw_state: torch.Tensor) -> torch.Tensor:
        z = self.net(gw_state)
        return torch.complex(-z.imag, z.real)   # pure 90° rotation
...
class SACRSN_Unified(nn.Module):
    """
    SACRSN Unified — the synthesis of v38 → v74.
    ...
    Five modules compete each T.O.T.E. cycle:
        ...
        3. Sensory (HebbianSensory)    — submodality enrichment
        4. Critic  (CriticModule)      — ethical / adversarial rotation

    Meta-head produces 4 values at each step:
        w_ethics      — scale critic influence  (ethical modulation)
    ...
    def _halt(self, vq_loss_sample: torch.Tensor,
        ...
        if self.training and self.train_step < 20:
            halt_prob = halt_prob * 0.1      # warmup: encourage exploration
    ...
    def forward_step(self, x_t: torch.Tensor, gw_state: torch.Tensor,
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            (v_out, vq_loss_s, indices, batch_ent,
             g_out, m_out, cand_mem, slot_ent,
             s_out, vis_attn, aud_attn, c_out) = self._step_modules(curr_state, mem_state)
            ...
            # ── Shadow Processing (Unconscious) ───────────────────────────
            shadow_probs = 1.0 - gates
            shadow_sum   = shadow_probs.sum(1, keepdim=True)
            shadow_norm  = torch.where(
                shadow_sum > 1e-6,
                shadow_probs / shadow_sum,
                torch.ones_like(shadow_probs) / shadow_probs.size(1)
            )
            shadow_c   = torch.complex(shadow_norm.unsqueeze(-1),
                                       torch.zeros_like(shadow_norm.unsqueeze(-1)))
            shadow_vec = (proposals * shadow_c).sum(dim=1)

            # Dynamic shadow weight scales with uncertainty
            w_unc              = meta_values[:, 3:4]
            dynamic_shadow     = CONFIG["shadow_weight"] * w_unc
            update             = winner_vec + dynamic_shadow * shadow_vec
            ...
            mask_vec   = active.view(-1)
            accum['phase'] += mask_vec * diff.mean(-1).view(-1) * valid.float().mean(-1).view(-1)
            ...
            # Update GW state and memory only for active samples
            ...
            mem_mask  = active.unsqueeze(-1)
            mem_state = mem_state + mem_mask * (cand_mem - mem_state)
...
def calculate_loss(logits: torch.Tensor, y: torch.Tensor,
    ...
    """
    Unified loss with linguistic, mathematical, and engineering terms.
    ...
    Components:
        ...
        ego_penalty    — metacognitive: high confidence on wrong answer → penalty
                         mirrors neural 'error monitoring' (ACC, dlPFC)
        depth_penalty  — T.O.T.E. depth in [depth_min, depth_max]
        vq_loss        — codebook commitment (symbolic grounding)
        ponder_cost    — penalise unnecessary extra thinking steps
        phase_reg      — penalise large phase jumps (coherence reward)
        entropy terms  — reward diverse VQ/slot usage, penalise gate collapse
    """
    ...
    if confidence is not None:
        ego_penalty = (confidence * loss_ce.detach()) * CONFIG["ego_penalty_weight"]

    # ── Depth Regularisation ──────────────────────────────────────────────
    ...
    if isinstance(stats.get('depth_per_sample'), torch.Tensor):
        ...
        depth_penalty = CONFIG['depth_target_weight'] * (
            F.relu(CONFIG['depth_min'] - d) + F.relu(d - CONFIG['depth_max'])
        ).mean()
...
def generate_text(model: SACRSN_Unified, tokenizer: RobustBPE,
    ...
    """
    model.eval()
    module_names = ["Gate", "Memory", "VQ", "Sensory", "Critic"]
...
def main():
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if torch.isnan(model.decoder.weight.grad).any():
                print(f"[!] NaN gradient in decoder at epoch {epoch}")
