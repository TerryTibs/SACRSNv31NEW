...
def check_nan(name: str, tensor: torch.Tensor, raise_on_nan: bool = True):
    """
    Diagnostic NaN/Inf probe — call after each major computation in
    forward_step to identify the FIRST point where NaN enters the graph.
    Set raise_on_nan=False for log-only mode during a discovery run.
    """
    if tensor.is_complex():
        bad = (torch.isnan(tensor.real) | torch.isinf(tensor.real) |
               torch.isnan(tensor.imag) | torch.isinf(tensor.imag)).any()
    else:
        bad = (torch.isnan(tensor) | torch.isinf(tensor)).any()
    if bad:
        print(f"[NaN/Inf DETECTED] {name}  shape={tuple(tensor.shape)}")
        if raise_on_nan:
...
CONFIG = {
    # ── Architecture ────────────────────────────────────────────────────────
    "seq_len":           128,   # FIXED from 32 — Tiny Shakespeare sentences run 80-120 chars;
                                 # 32 tokens is too short to see a complete sentence.
    "batch_size":        32,
    "embed_dim":         128,   # FIXED from 64 — ~4K vocab needs wider embedding space.
                                 # 64-dim gives only ~16 usable directions statistically.
    "mem_slots":         32,
    "mem_topk":          3,
    "vocab_size":        None,  # Set automatically after tokeniser trains
    "codebook_size":     None,  # Set automatically: ~20% of vocab, power-of-2, clamped [64,1024]
    "n_sensory_anchors": 32,

    # ── Cognitive Dynamics (T.O.T.E. loop) ───────────────────────────────
    "max_recursion":     5,
    "halt_bias_init":    1.86,  # FIXED from 6.0. At bias=6/scale=4, halt_prob=0.88 at
                                 # norm_loss=1.0 → expected depth=1.14 (model halts at step 1
                                 # on virtually every token — barely better than a single-step model).
                                 #
                                 # Target: depth ≈ 2.5 at norm_loss=1.0 (average difficulty),
                                 # rising to ~4.7 at norm_loss=2.0 (very hard tokens).
                                 # Formula: sigmoid(softplus(bias) - softplus(scale) × norm) = 0.36
                                 # → softplus(bias) = softplus(scale) - 0.575
                                 # → bias_init=1.86, scale_init=2.5 gives:
                                 #   norm=0.5: depth≈2.3  norm=1.0: depth≈2.8
                                 #   norm=1.5: depth≈4.1  norm=2.0: depth≈4.7
    "halt_scale_init":   2.5,   # FIXED from 4.0. Shallower sigmoid → halt responds
                                 # proportionally to VQ quality across the full difficulty range.
    "ponder_cost":       0.001, # Keep reduced — effort signal without fighting CE.
    ...
    "module_div_weight":    0.02,  # Penalise pairwise cosine similarity between module
                                   # proposals — prevents all 5 modules collapsing to the
                                   # same transformation. 0.02 is ~10% of CE scale.

    # ── Depth (T.O.T.E. step count) Targets ──────────────────────────────
    "depth_target_weight": 0.1,   # FIXED from 0.05 — stronger pull to help halt training
    "depth_min":           1.0,   # FIXED from 2.0 — allow halting on trivial tokens
    "depth_max":           5.0,   # FIXED from 6.0 — match max_recursion
    "target_depth":        3.0,   # FIXED from 4.0 — more achievable with new halt params

    # ── Metacognition / Neurolinguistics ──────────────────────────────────
    "ego_penalty_weight":  0.05,  # FIXED from 0.5 — 10× reduction. At 0.5, mid-training
                                   # ego_penalty = conf(0.4)×CE(2.0)×0.5 = 0.4 ≈ CE itself.
                                   # Loss becomes dominated by metacognition, not language.
    "synaptic_decay":      1e-5,
    "surprise_decay":      0.95,  # FIXED from 0.9 — 0.9 tracks ~10 batches, too noisy.
                                   # 0.95 smooths over ~20 batches for stable plasticity.
    "active_plasticity":   True,

    # ── Training ──────────────────────────────────────────────────────────
    "epochs":       2000,
    "lr":           1e-4,
    "grad_clip":    0.5,   # MISSING — was referenced in training loop (L1689) but not
                            # defined in CONFIG, causing a KeyError crash on first training step.
    "replay_size":  512,   # FIXED from 200 — too small for diverse sleep consolidation.
    "sleep_every":  50,
}
...
class StableComplexNorm(nn.Module):
    """
    Normalise real and imaginary components independently with safe backward.

    WHY nn.LayerNorm IS DANGEROUS HERE:
        nn.LayerNorm backward gradient = (1/sqrt(var+eps)) * (...).
        At var=0 (all-zero memory slots at init): 1/sqrt(1e-6) = 316×.
        Memory starts as zeros, so this 316× amplifier fires on epoch 0
        and causes the AddBackward0 NaN traced to AssociativeMemory line 520.

    FIX — same pattern as ComplexLayerNorm:
        Detach the variance so it acts as a fixed scaling constant for
        this step. The (x - mean) path still receives full gradients.
        eps=1e-4 (not 1e-6) as additional safety margin for near-zero memory.

    NOTE: This norm distorts phase angle (Re and Im normalised separately),
    so it must NOT be used in the main TOTE loop. Use ComplexLayerNorm there.
    For AssociativeMemory slot stabilisation the phase distortion is acceptable.
    """
    def __init__(self, dim: int):
        super().__init__()
        self.scale_r = nn.Parameter(torch.ones(dim))
        self.bias_r  = nn.Parameter(torch.zeros(dim))
        self.scale_i = nn.Parameter(torch.ones(dim))
        self.bias_i  = nn.Parameter(torch.zeros(dim))

    def _safe_norm(self, x: torch.Tensor,
                   scale: nn.Parameter, bias: nn.Parameter) -> torch.Tensor:
        mean = x.mean(dim=-1, keepdim=True)
        # Detach variance: prevents 1/sqrt(var+eps) from entering backward graph.
        var  = x.detach().var(dim=-1, keepdim=True)
        return (x - mean) / (var + 1e-4).sqrt() * scale + bias

    def forward(self, z: torch.Tensor) -> torch.Tensor:
        return torch.complex(
            self._safe_norm(z.real, self.scale_r, self.bias_r),
            self._safe_norm(z.imag, self.scale_i, self.bias_i)
        )
...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # clamp(min=1e-3): hard floor on denominator — even with var detached,
        # a pathological all-identical-magnitude input gives var=0, denom=0.01.
        # 1e-3 prevents any normalised value exceeding ±100 before scale/shift.
        denom    = (var + 1e-4).sqrt().clamp(min=1e-3)
        norm_mag = (mag - mean) / denom

        # tanh: soft-clips the normalised magnitude to (-1, 1) before learnable
        # rescaling. Prevents scale parameter from amplifying outlier magnitudes
        # into the explosive range during deep recursion.
        norm_mag = torch.tanh(norm_mag)

        # clamp scale/shift: if scale or shift parameters drift large (possible
        # in early training with high LR), they can undo the tanh bounding above.
        # ±5 allows meaningful gain control while preventing runaway amplification.
        norm_mag = norm_mag * self.scale.clamp(-5.0, 5.0) + self.shift.clamp(-5.0, 5.0)
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state: torch.Tensor, prev_mem: torch.Tensor):
        # TRUNCATED BPTT — detach prev_mem entirely.
        #
        # Memory is an RNN-style recurrence: next_mem depends on prev_mem which
        # depends on the prev_mem before that, unrolled across every TOTE step
        # and every token (up to max_recursion × seq_len = 5×64 = 320 steps).
        # StableComplexNorm applies a ~100× gradient scale at near-zero memory
        # (1/sqrt(var+1e-4) = 1/0.01 = 100). Over 320 steps: 100^320 → ∞.
        #
        # Detaching prev_mem truncates this chain at the current step.
        # Gradients still train gate_net and address_net because write_gate and
        # sparse both derive from the current gw_state (which is in-graph).
        # read_out gradients flow through attn back to gw_state normally.
        # This is standard practice for memory-augmented networks (NTM, DNC).
        mem = prev_mem.detach()

        # ── READ: Hermitian cosine similarity ────────────────────────────
        q        = gw_state.unsqueeze(1)                   # (B, 1, d)
        sim      = (mem.conj() * q).real.sum(dim=-1)       # (B, slots)
        sim      = sim - sim.max(dim=-1, keepdim=True)[0].detach()
        attn     = F.softmax(sim, dim=-1).unsqueeze(-1)
        read_out = (mem * torch.complex(attn, torch.zeros_like(attn))).sum(1)
        ...
        effective_update = write_gate * sparse.unsqueeze(-1)        # MulBackward0
        new_slot         = gw_state.unsqueeze(1)
        next_mem         = (1.0 - effective_update) * mem + \
                           effective_update * new_slot              # AddBackward0
    ...
    def forward(self, z: torch.Tensor):
        ...
        if self.training:
            with torch.no_grad():
                ...
                # +1e-6 smoothly prevents divide-by-zero without the hard
                # discontinuity of clamp(min=1.0), which can create gradient
                # spikes at the clamp boundary during early EMA updates.
                cluster_size = self.ema_cluster_size + self.epsilon + 1e-6
                ...
                # Improvement 8 — dead code restart.
                # Codes with cluster_size < 1 haven't been assigned in recent batches.
                # Reinitialise them to a random encoder output so they re-enter
                # competition rather than accumulating stale near-zero embeddings.
                dead = self.ema_cluster_size < 1.0
                n_dead = int(dead.sum().item())
                if n_dead > 0 and z_flat.size(0) >= n_dead:
                    rand_idx = torch.randint(0, z_flat.size(0), (n_dead,),
                                             device=z_flat.device)
                    revived  = z_flat[rand_idx].detach()
                    self.embedding.weight.data[dead] = revived
                    self.ema_w[dead]                 = revived
...
class SACRSN_Unified(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int):
        ...
        self.register_buffer('halt_temp',   torch.tensor(0.5))
        # Cache identity for orthogonality loss — avoids 20K+ allocations per epoch
        # (batch × seq × max_recursion = 32 × 128 × 5 if allocated inside TOTE loop)
        self.register_buffer('mod_identity', torch.eye(5).unsqueeze(0))

        self.apply(self._init_weights)

        # Improvement 4 — tied encoder/decoder weights.
        # encoder.weight shape: (vocab, dim*2)
        # decoder.weight shape: (vocab, dim*2)  [Linear stores (out, in)]
        # Tying halves I/O parameter cost and forces a shared embedding manifold.
        #
        # CHECKPOINT RESTORE CAVEAT: torch.save(model.state_dict()) stores
        # encoder.weight and decoder.weight as separate tensors. After
        # load_state_dict() the tie is broken — they become independent again.
        # To restore correctly: call model.decoder.weight = model.encoder.weight
        # immediately after load_state_dict().
    ...
    def _arbitrate(self, curr_state: torch.Tensor,
        ...
        # Ethical modulation: push quadrature channel (4) when ethics is high
        ...
        # Hard clamp before max-subtraction: prevents ±inf feeding into softmax
        # if any upstream module produced extreme values.
        raw_logits = raw_logits.clamp(-50.0, 50.0)

        raw_logits = raw_logits - raw_logits.max(dim=-1, keepdim=True)[0].detach()
        check_nan("arb_logits_pre_softmax", raw_logits, raise_on_nan=False)
    ...
    def _halt(self, vq_loss_sample: torch.Tensor,
        ...
        # Improvement 9 — temperature annealing.
        # halt_temp starts at 0.5 (halves halt_prob → deeper exploration in early
        # training) and anneals to 1.0 by step ~500 (full halt sensitivity).
        # The training loop increments halt_temp by 0.001/step until it reaches 1.0.
        halt_prob = (halt_prob * self.halt_temp).clamp(0.0, 1.0)
    ...
    def forward_step(self, x_t: torch.Tensor, gw_state: torch.Tensor,
        ...
        accum = {k: torch.zeros(B, device=DEVICE)
                 for k in ['vq', 'ponder', 'phase', 'module_div']}
        ...
        steps_taken = 0
        gw_mag_sum  = 0.0
        gw_out      = gw_state       # in-graph output; updated each TOTE step
        for i in range(CONFIG["max_recursion"]):
            steps_taken += 1
            curr_state  = self.norm(gw_state)
            mask_vec    = active.view(-1)   # defined first — used throughout loop
            ...
            # ── NaN Instrumentation (raise_on_nan=False → log and continue) ──
            check_nan("g_out",     g_out,     raise_on_nan=False)
            check_nan("m_out",     m_out,     raise_on_nan=False)
            check_nan("v_out",     v_out,     raise_on_nan=False)
            check_nan("s_out",     s_out,     raise_on_nan=False)
            check_nan("q_out",     q_out,     raise_on_nan=False)
            check_nan("vq_loss_s", vq_loss_s, raise_on_nan=False)
            ...
            # ── Module Specialisation — Orthogonality Loss ────────────────
            # Goal: keep the 5 module outputs as linearly independent as possible
            # so the arbitrator always has meaningfully different proposals to
            # choose between. If modules collapse, GWT competition is meaningless.
            #
            # Previous implementation had three bugs:
            #   Bug 1 — Dead code: no_grad block computed props_flat/props_norm
            #            and then threw them away; in-graph version recomputed
            #            the same thing immediately after.
            #   Bug 2 — Gradient degeneracy: cosine_sim gradient at identical
            #            vectors = (a-a)/||a|| = 0. The penalty was maximum (1.0)
            #            but the gradient was zero — no push to separate.
            #   Bug 3 — Wrong sign: adding raw cosine similarity to loss
            #            rewards anti-correlation (sim=-1) equally to independence
            #            (sim=0). We want independence, not opposition.
            #
            # Correct formulation: Gram-matrix orthogonality loss.
            #
            #   A   = (B, 5, 2d)  normalised module outputs
            #   G   = A @ Aᵀ      (B, 5, 5) Gram matrix
            #   At perfect orthogonality: G = I (identity)
            #   Loss = ||G - I||_F² / 25   (normalise by #elements)
            #
            # Properties:
            #   • Value = 0 when all 5 modules are mutually orthogonal (ideal)
            #   • Value > 0 whenever any pair shares directional similarity
            #   • Gradient = 4*(G - I)@A — nonzero whenever G ≠ I, including
            #     when two modules are identical (G[i,j]=1, grad = 4*(1-δ_ij)*A[j])
            #   • No gradient degeneracy at any configuration
            #   • Symmetric: penalises both positive AND negative correlation
            #     equally (off-diagonal of G-I matters both ways)
            props_flat = torch.cat([proposals.real, proposals.imag], dim=-1)  # (B,5,2d)
            props_norm = F.normalize(props_flat, dim=-1)                       # unit norm
            gram       = torch.bmm(props_norm, props_norm.transpose(1, 2))     # (B,5,5)
            orth_loss  = (gram - self.mod_identity).pow(2).mean(dim=(-1, -2))  # (B,)  /25
            accum['module_div'] += mask_vec * orth_loss

            gates_c    = torch.complex(gates.unsqueeze(-1),
                                       torch.zeros_like(gates.unsqueeze(-1)))
            winner_vec = (proposals * gates_c).sum(dim=1)
            check_nan("winner_vec", winner_vec, raise_on_nan=False)
            ...
            check_nan("cand_state", cand_state, raise_on_nan=False)
            ...
            with torch.no_grad():
                mag        = (cand_state.real.pow(2) + cand_state.imag.pow(2)).sqrt()
                valid      = mag > 1e-6
                # Safe angle: atan2(imag+eps, real+eps) avoids atan2(0,0)=undefined.
                # Using atan2 directly rather than .angle() for explicit eps control.
                safe_angle = torch.where(
                    valid,
                    torch.atan2(cand_state.imag + 1e-8, cand_state.real + 1e-8),
                    prev_angle
                )
            ...
            # TWO SEPARATE VARIABLES — this is the critical design point:
            #
            # gw_out  = in-graph cand_state → returned → appended to outputs
            #           → decoder → CE loss → GRADIENTS FLOW TO ALL PARAMS
            #
            # gw_state = gw_out.detach() → fed into NEXT TOTE step as input
            #           → prevents backward unrolling 5 steps × 100× norm = 100^5
            #
            # Without this split, either:
            #   (a) detach everything → decoder sees dead tensors, GradNorm≈0.25
            #       but loss never moves (the bug that caused TextPPX to plateau)
            #   (b) detach nothing → 100^5 gradient explosion → NaN
            gw_out = torch.complex(
                torch.where(active > 0.5, cand_state.real, gw_out.real),
                torch.where(active > 0.5, cand_state.imag, gw_out.imag)
            )                        # in-graph — for decoder output
            gw_state = gw_out.detach()  # detached — for next TOTE step input
            ...
            # Detach mem_state update: memory module already detaches prev_mem
            # internally, but the mem_mask interpolation here adds another
            # gradient path. Detach the incoming mem_state before the update
            # so the TOTE-step recurrence doesn't accumulate backward graphs.
            mem_state  = mem_state.detach() + mem_mask_c * (cand_mem - mem_state.detach())

            active = active * (1.0 - should_stop)
            with torch.no_grad():
                gw_mag_sum += (gw_state.real.pow(2) + gw_state.imag.pow(2)).sqrt().mean().item()
            if active.sum() == 0:
                break
        ...
        avg_gw_mag   = gw_mag_sum / max(1, steps_taken)   # diagnostic

        return (gw_out, mem_state, accum, final_indices,   # gw_out is in-graph
    ...
    def forward(self, x_seq: torch.Tensor, mem_state=None):
        ...
        # Improvement 1 — sinusoidal position encoding.
        # Without PE, token position 5 and position 50 look identical — the model
        # processes sequences as unordered bags of states. For Shakespeare, word
        # order is everything ("to be or not to be" ≠ "be to not or be to").
        #
        # Complex PE: Real channel carries standard sin/cos positional signal.
        # Imaginary channel is phase-shifted by π/2 (cos/−sin) to keep the two
        # channels orthogonal in ℂ. Scale 0.1 keeps PE smaller than embedding
        # magnitudes (~0.02 std) so it adds position without overwhelming content.
        #
        # Requires even embed_dim (0::2 and 1::2 slices must be equal length).
        assert self.dim % 2 == 0, \
            f"embed_dim must be even for sinusoidal PE, got {self.dim}"
        pos = torch.arange(T, device=x_seq.device).unsqueeze(0).float()  # (1, T)
        div = torch.exp(
            torch.arange(0, self.dim, 2, device=x_seq.device).float()
            * -(math.log(10000.0) / self.dim)
        )                                                                   # (dim/2,)
        pe_real = torch.zeros(1, T, self.dim, device=x_seq.device)
        pe_imag = torch.zeros(1, T, self.dim, device=x_seq.device)
        pe_real[..., 0::2] = torch.sin(pos.unsqueeze(-1) * div)
        pe_real[..., 1::2] = torch.cos(pos.unsqueeze(-1) * div)
        pe_imag[..., 0::2] = torch.cos(pos.unsqueeze(-1) * div)   # π/2 phase shift
        pe_imag[..., 1::2] = -torch.sin(pos.unsqueeze(-1) * div)
        gw_seq = gw_seq + 0.1 * torch.complex(pe_real, pe_imag)
        ...
        for t in range(T):
            x_t = gw_seq[:, t]
            # forward_step returns gw_out (in-graph cand_state) as first element.
            # We append it to outputs so the decoder has a live gradient path.
            # We then detach it before passing it as gw_state to the next token
            # to truncate the cross-token recurrence chain (TBPTT).
            (gw_out, mem_state, step_stats, indices,
             batch_ent, gate_ent, slot_ent, depth,
             meta_vals, wins, sensory_ent, gw_mag) = self.forward_step(
                 x_t, gw_state, mem_state.detach())

            outputs.append(gw_out)          # in-graph → gradient reaches all params
            gw_state = gw_out.detach()      # detached → no cross-token BPTT explosion
            ...
            stats['gw_mag']      += gw_mag    # diagnostic: workspace magnitude
...
def calculate_loss(logits: torch.Tensor, y: torch.Tensor,
    ...
    Components:
        ...
        module_div     — penalise pairwise cosine similarity between module proposals
    """
    ...
    if confidence is not None:
        # confidence.detach(): prevents second-order gradient amplification.
        # Both confidence and loss_ce carry gradients; their product creates
        # a d(loss)/d(params) term that scales quadratically if confidence spikes.
        # Detaching confidence keeps the CE gradient clean while still penalising
        # high-confidence errors as a training signal.
        ego_penalty = confidence.detach() * loss_ce * CONFIG["ego_penalty_weight"]
    ...
    struct_loss = struct_scale * (
        ...
        # ── Module specialisation — penalise identical module outputs ─────
        + CONFIG['module_div_weight']    * stats['module_div']
...
def visualise_suite(model: SACRSN_Unified, history: dict,
    ...
    with torch.no_grad():
        ...
        rs, ims = [], []
        for _ in range(30):
            gw, mem, _, _, _, _, _, _, _, _, _, _ = model.forward_step(xt, gw, mem)
...
def main():
    ...
    corpus_chars = len(data_text)

    # ── Auto-size vocab from corpus ───────────────────────────────────────
    # Rule of thumb: larger corpus → more subword merges are statistically
    # reliable. Scale log-linearly from 512 (≤10 KB) to 8000 (≥10 MB),
    # then round to the nearest 128 for clean embedding table alignment.
    #
    #   chars   →  target_vocab
    #   10 000  →   ~800
    #   50 000  →  ~1400
    ...
    #    1 MB   →  ~3800
    #   10 MB   →  ~8000
    raw_vocab_target = 512 * (corpus_chars / 10_000) ** 0.45
    target_vocab     = int(round(max(512, min(8000, raw_vocab_target)) / 128) * 128)
    print(f"  Corpus: {corpus_chars:,} chars → target vocab: {target_vocab:,}")

    tokenizer = RobustBPE(target_vocab_size=target_vocab)
    tokenizer.train(data_text)
    CONFIG["vocab_size"] = len(tokenizer.vocab)

    # ── Auto-size codebook from vocab ─────────────────────────────────────
    # Codebook represents the number of discrete concept-clusters the VQ
    # layer can form. Heuristic: ~20 % of vocab, clamped to [64, 1024] and
    # rounded to a power of 2 for efficient GPU utilisation.
    #
    #   vocab  →  codebook
    #    512   →    128
    #   1000   →    256
    ...
    raw_cb   = CONFIG["vocab_size"] * 0.20
    exp      = _math.log2(max(64, min(1024, raw_cb)))
    codebook = int(2 ** round(exp))
    CONFIG["codebook_size"] = codebook
    print(f"  Vocab: {CONFIG['vocab_size']:,} → codebook size: {codebook:,}")

    data_ids    = tokenizer.encode(data_text)

    # Improvement 2 — validation split (90 / 10, sequential).
    # Hold out the last 10% of tokens. Never trained on — measures generalisation.
    n_val       = max(CONFIG["seq_len"] * CONFIG["batch_size"], len(data_ids) // 10)
    train_ids   = data_ids[:-n_val]
    val_ids     = data_ids[-n_val:]
    data_tensor = torch.tensor(train_ids, dtype=torch.long)
    ...
    # Improvement 7 — per-group learning rates.
    # I/O: simple embedding lookup / linear — can move fast.
    # cognitive_slow: recurrent modules (gate, memory) — gentle to preserve dynamics.
    # cognitive_fast: sensory, quadrature, arbitration, norm — moderate.
    # VQ embedding has requires_grad=False (pure EMA) — excluded automatically.
    io_params    = list(model.encoder.parameters())
    dec_bias     = [model.decoder.bias] if model.decoder.bias is not None else []
    slow_params  = (list(model.gate_mod.parameters()) +
                    list(model.memory_module.parameters()))
    fast_params  = (list(model.sensory_cortex.parameters()) +
                    list(model.quadrature.parameters()) +
                    list(model.arbitrator.parameters()) +
                    list(model.meta_head.parameters()) +
                    list(model.norm.parameters()) +
                    [model.halt_bias, model.halt_scale, model.input_gate])
    opt = torch.optim.AdamW([
        {'params': io_params + dec_bias, 'lr': 2e-4,  'name': 'io'},
        {'params': slow_params,          'lr': 5e-5,  'name': 'cognitive_slow'},
        {'params': fast_params,          'lr': 1e-4,  'name': 'cognitive_fast'},
    ], weight_decay=1e-2)

    # Improvement 5 — LR warmup then cosine annealing.
    # 100 epochs of linear warmup (1% → 100%) avoids large noisy gradients
    # from random weights locking parameters into bad basins.
    warmup_epochs = 100
    warmup_sched  = torch.optim.lr_scheduler.LinearLR(
        opt, start_factor=0.01, end_factor=1.0, total_iters=warmup_epochs)
    cosine_sched  = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt, T_max=max(1, CONFIG["epochs"] - warmup_epochs), eta_min=1e-6)
    ...
    print(f"  Parameters: {n_params:,}  (encoder/decoder tied)")
    ...
    epochs_no_improve = 0
    EARLY_STOP_PATIENCE = 300   # epochs without val improvement before stopping
    VAL_EVERY           = 50    # evaluate val loss every N epochs
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            # NOTE: decoder is excluded — decoder.weight is tied to encoder.weight;
            # applying decay here would also decay the embeddings, doubling the
            # L2 effect already applied by the optimizer's weight_decay=1e-2.
            with torch.no_grad():
                for layer in [model.arbitrator, model.meta_head]:
            ...
            # Log total norm before clip so you can see how large it actually is.
            with torch.no_grad():
                grads = [p.grad.detach().norm(2)
                ...
                if torch.isnan(total_norm):
                    print(f"[!] Gradient norm is NaN at epoch {epoch} — backward exploded")
            ...
            # Improvement 9 — anneal halt temperature toward 1.0.
            # Increment by 0.001 per epoch: reaches 1.0 after ~500 epochs.
            if model.halt_temp.item() < 1.0:
                model.halt_temp.add_(0.001).clamp_(0.0, 1.0)  # add THEN clamp
            ...
            # Improvement 2 — validation loss + early stopping.
            if epoch % VAL_EVERY == 0 and epoch > 0:
                model.eval()
                with torch.no_grad():
                    vx, vy   = get_batch(val_tensor, CONFIG["batch_size"],
                                         CONFIG["seq_len"])
                    vlogits, vstats, _, _, vmeta, _ = model(vx)
                    val_ce   = F.cross_entropy(
                        vlogits.reshape(-1, CONFIG["vocab_size"]), vy.reshape(-1)
                    ).item()
                    val_ppx  = math.exp(min(100, val_ce))
                    history['val_loss'].append(val_ce)
                    history['val_ppx'].append(val_ppx)

                    if val_ce < best_val_loss:
                        best_val_loss     = val_ce
                        epochs_no_improve = 0
                        # Improvement 3 — save best checkpoint
                        torch.save({
                            'epoch':       epoch,
                        ...
                        print(f"  ★ New best val loss: {val_ce:.4f}  "
                              f"(PPX {val_ppx:.1f}) — saved checkpoint_best.pt")
                    else:
            ...
            # Improvement 3 — periodic checkpoint (regardless of val improvement).
            ...
            if epoch % 20 == 0:
                ...
                if history['grad_norms']:
                    ...
                    val_str  = (f"  ValPPX: {history['val_ppx'][-1]:.1f}"
                                if history.get('val_ppx') else "")
                    print(f"         Dominant module: [{dominant}]  "
                          f"Confidence: {confidence.item():.3f}  "
                          f"GradNorm: {total_norm.item():.3f}"
                          f"{val_str}")
