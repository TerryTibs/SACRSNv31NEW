...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t: torch.Tensor, gw_state: torch.Tensor,
        ...
        steps_taken = 0
        gw_mag_sum  = 0.0          # for diagnostic logging
        for i in range(CONFIG["max_recursion"]):
            ...
            # Update GW state only for active samples, then detach.
            # The TOTE loop is also an RNN (up to 5 steps). Without detach,
            # backward unrolls through every step × ComplexLayerNorm (100×) = 100^5.
            # Detaching keeps the per-step gradient (curr_state → modules → cand_state)
            # without unrolling the full TOTE chain. Each step still trains all modules.
    ...
    def forward(self, x_seq: torch.Tensor, mem_state=None):
        ...
        for t in range(T):
            x_t = gw_seq[:, t]
            # Detach BOTH gw_state and mem_state at every token boundary.
            # gw_state is an RNN hidden state: unrolled over T=64 tokens × 5 TOTE
            # steps = 320 backward passes through ComplexLayerNorm (~100× at near-zero).
            # 100^320 overflows float32 immediately. Truncated BPTT (detach at each
            # token boundary) keeps gradients within one token's TOTE window.
            # Gradient still flows from outputs[t] → logits[:, t, :] → CE loss,
            # giving full supervision at every token position.
            (gw_state, mem_state, step_stats, indices,
             ...
             meta_vals, wins, sensory_ent, gw_mag) = self.forward_step(
                 x_t, gw_state.detach(), mem_state.detach())
