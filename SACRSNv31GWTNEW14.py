...
print(f"--- SACRSN v48: FINAL STABLE EDITION ---")
...
class SACRSN_v48(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Differentiable Halting (STE)
            # Hard decision forward, Prob gradient backward
            ...
            # [CRITICAL FIX] Correct Broadcasting for Memory Mask
            mem_mask = active_mask.unsqueeze(-1) # (B, 1, 1)
...
def main():
    ...
    model = SACRSN_v48(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
