"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v85 Enhanced Learning)              ║
...
# ═══════════════════════════════════════════════════════════════════════════
# 0.  STRICT DETERMINISM & DEVICE CONFIGURATION
# ═══════════════════════════════════════════════════════════════════════════
SEED = 42
...
CONFIG = {
    ...
    "vq_decay":          0.99,
    "dead_code_threshold": 1.0, # [NEW] Restart codes used less than this

    # ── Regularisation ───────────────────────────────────────────────────
    "entropy_weight_start": 0.10,
    "entropy_weight_end":   0.02,
    "gate_sparsity_weight": 0.01,
    "slot_balance_weight":  0.005,
    "phase_reg":            0.005,
    "sensory_reg":          0.005,
    "module_diversity_weight": 0.1, # [NEW] Force modules to differ
    ...
    # ── Training & Plasticity ────────────────────────────────────────────
    "epochs":            2000,
    "lr_base":           2e-4,  # [NEW] Base LR for IO
    "lr_cognitive":      5e-5,  # [NEW] Slower LR for internal modules
    "grad_clip":         0.5,
    "replay_size":       512,
    "sleep_every":       50,
    "ego_penalty_weight":0.05,
    "synaptic_decay":    1e-5,
    ...
    "patience":          20,    # Validation checks before stopping
    "val_interval":      50,    # Check val every X epochs
    "warmup_steps":      100,   # Linear warmup
}

print(f"--- SACRSN v85: ENHANCED LEARNING EDITION ---")
...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Algebraic Phase Preservation z/|z|
        z_unit = z / (mag + 1e-9)
        
        # Tanh clamp after norm for safety
        norm_mag = torch.tanh(norm_mag)
        
        return torch.complex(norm_mag * z_unit.real, norm_mag * z_unit.imag)
...
class HermitianGating(nn.Module):
    ...
    def forward(self, x):
        ...
        score_r = (q * k.conj()).real.sum(dim=-1, keepdim=True) * self.scale
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        mem = prev_mem.detach() # Truncated BPTT for memory safety

        q = gw_state.unsqueeze(1)
        sim = (mem.conj() * q).real.sum(dim=-1)
        ...
        delta = effective_update * (new_slot - mem)
        next_mem = mem + delta
...
class EMA_VQ(nn.Module):
    def __init__(self, dim, num_concepts):
        ...
        # [FIX] Freeze embedding gradients (EMA only)
        ...
        # [NEW] Track usage for dead code restart
        self.register_buffer('usage_count', torch.zeros(num_concepts))
    ...
    def forward(self, z):
        ...
        if self.training:
            with torch.no_grad():
                ...
                # [NEW] Update usage counts for restart logic
                ...
                # [NEW] Dead Code Restart (every 100 steps)
                # If a code hasn't been used, reset it to a random encoder output
                if random.random() < 0.01:
                     dead_indices = (self.usage_count < CONFIG["dead_code_threshold"]).nonzero(as_tuple=True)[0]
                     if dead_indices.numel() > 0:
                         # Pick random vectors from current batch
                         rand_idx = torch.randint(0, z_flat.size(0), (dead_indices.numel(),), device=DEVICE)
                         replacements = z_flat[rand_idx].detach()
                         self.embedding.weight.data[dead_indices] = replacements
                         self.ema_w.data[dead_indices] = replacements * self.ema_cluster_size[dead_indices].unsqueeze(1)
                         self.usage_count.fill_(0) # Reset counters
...
class SACRSN_Unified(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [NEW] Tied Weights: Decoder weights = Encoder weights transpose
    ...
    def _halt(self, vq_loss, meta, epoch_prog):
        ...
        # [NEW] Temperature Annealing for Halting
        # Start at 0.5 (exploration), anneal to 1.0 (exploitation)
        temp = 0.5 + 0.5 * min(1.0, epoch_prog * 2) # Saturated by 50% training
        halt_prob = halt_prob * temp
    ...
    def forward_step(self, x_t, gw_state, mem_state, epoch_prog=0.0):
        # [NEW] Sinusoidal Position Encoding added in forward(), x_t already has it
        alpha = torch.sigmoid(self.input_gate)
        gw_state = alpha * gw_state + (1.0 - alpha) * x_t
        
        gw_state = torch.complex(torch.tanh(gw_state.real), torch.tanh(gw_state.imag))
        
        B = x_t.size(0)
        active = torch.ones(B, 1, device=DEVICE)
        accum = {k: torch.zeros(B, device=DEVICE) for k in ['vq', 'ponder', 'phase', 'div']}
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [NEW] Module Diversity Loss
            # Calculate cosine similarity between proposals to prevent collapse
            if active.sum() > 0:
                props_flat = torch.cat([proposals.real, proposals.imag], dim=-1) # (B, 5, 2D)
                props_norm = F.normalize(props_flat, dim=-1)
                sim_matrix = torch.bmm(props_norm, props_norm.transpose(1, 2)) # (B, 5, 5)
                mask = torch.triu(torch.ones(5, 5, device=DEVICE), diagonal=1).bool()
                div_loss = sim_matrix[:, mask].mean(dim=-1) # (B)
                accum['div'] += active.view(-1) * div_loss
            
            gates_c = torch.complex(gates.unsqueeze(-1), torch.zeros_like(gates.unsqueeze(-1)))
            winner = (proposals * gates_c).sum(dim=1)
            ...
            update = winner + dyn_shadow * shadow_prop
            ...
            c_r = torch.nan_to_num(cand.real, nan=0.0, posinf=1.0, neginf=-1.0)
            c_i = torch.nan_to_num(cand.imag, nan=0.0, posinf=1.0, neginf=-1.0)
            cand = torch.complex(c_r, c_i)
            ...
            stop = self._halt(vq_l, meta, epoch_prog)
            ...
            act_det = active.detach().squeeze(-1)
            gw_r = gw_state.real * (1.0 - act_det) + cand.real * act_det
            gw_i = gw_state.imag * (1.0 - act_det) + cand.imag * act_det
            gw_state = torch.complex(gw_r, gw_i)
            
            mem_m = active.detach().unsqueeze(-1)
            mem_r = mem_state.real * (1.0 - mem_m) + c_mem.real * mem_m
            mem_i = mem_state.imag * (1.0 - mem_m) + c_mem.imag * mem_m
            mem_state = torch.complex(mem_r, mem_i)
    ...
    def forward(self, x_seq, mem_state=None, epoch_prog=0.0):
        B, T = x_seq.shape
        emb = self.encoder(x_seq)
        gw_seq = torch.complex(emb[..., :self.dim], emb[..., self.dim:])
        
        # [NEW] Sinusoidal Position Encoding
        pos = torch.arange(T, device=DEVICE).unsqueeze(0) # (1, T)
        div_term = torch.exp(torch.arange(0, self.dim, 2, device=DEVICE) * -(math.log(10000.0) / self.dim))
        pe_r = torch.zeros(1, T, self.dim, device=DEVICE)
        pe_r[..., 0::2] = torch.sin(pos.unsqueeze(-1) * div_term)
        pe_r[..., 1::2] = torch.cos(pos.unsqueeze(-1) * div_term)
        pe_i = torch.roll(pe_r, 1, dims=-1)
        gw_seq = gw_seq + 0.1 * torch.complex(pe_r, pe_i)
        
        if mem_state is None: mem_state = self.memory_module.init_state(B)
        gw = torch.zeros_like(gw_seq[:, 0])
        
        outputs, idxs = [], []
        stats = {k: 0.0 for k in ['vq','ponder','phase','ent','gate_ent','slot_ent','depth','sensory_ent','div']}
        ...
        for t in range(T):
            ...
            (gw, mem_state, step_stats, idx, b_e, g_e, s_e, d, m_v, w, se) = \
                self.forward_step(x_t, gw, mem_state, epoch_prog)
                
            outputs.append(gw)
            idxs.append(idx)
            for k in ['vq', 'ponder', 'phase', 'div']: stats[k] += step_stats[k].mean()
            stats['ent'] += b_e
            stats['gate_ent'] += g_e
            stats['slot_ent'] += s_e
            stats['depth'] += d
...
def generate_text(model, tokenizer, seed="The", length=60, verbose=False):
    ...
    with torch.no_grad():
        try:
            _, _, mem, _, _, _ = model(curr, mem, 1.0)
            curr = curr[:, -1:]
            for i in range(length):
                logits, _, mem, _, _, wins = model(curr, mem, 1.0)
...
def calc_loss(logits, y, stats, meta, plast, ep, tot_ep):
    ...
    struct_loss = struct * (
        ...
        CONFIG['module_diversity_weight'] * stats['div'] +
        d_pen
    )
...
def main():
    ...
    # [NEW] Validation Split (90/10)
    split_idx = int(len(txt) * (1 - CONFIG["val_split"]))
    train_txt, val_txt = txt[:split_idx], txt[split_idx:]
    
    train_data = torch.tensor(tok.encode(train_txt), dtype=torch.long)
    val_data = torch.tensor(tok.encode(val_txt), dtype=torch.long)
    print(f"Train Tokens: {len(train_data)} | Val Tokens: {len(val_data)}")

    model = SACRSN_Unified(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    
    # [NEW] Per-Group Learning Rates
    opt = torch.optim.AdamW([
        {'params': [*model.encoder.parameters(), *model.decoder.parameters()], 'lr': CONFIG["lr_base"]},
        {'params': [*model.gate_mod.parameters(), *model.memory_module.parameters(), 
                    *model.sensory_cortex.parameters(), *model.quadrature.parameters(),
                    *model.arbitrator.parameters(), *model.meta_head.parameters(), 
                    model.halt_bias, model.halt_scale, model.input_gate], 'lr': CONFIG["lr_cognitive"]},
    ], lr=CONFIG["lr_base"])

    # [NEW] Warmup Scheduler
    warmup = torch.optim.lr_scheduler.LinearLR(opt, start_factor=0.01, end_factor=1.0, total_iters=CONFIG["warmup_steps"])
    cosine = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=CONFIG["epochs"] - CONFIG["warmup_steps"])
    sched = torch.optim.lr_scheduler.SequentialLR(opt, schedulers=[warmup, cosine], milestones=[CONFIG["warmup_steps"]])
    ...
    try:
        for ep in range(CONFIG["epochs"]):
            ...
            epoch_prog = ep / CONFIG["epochs"]
            logits, stats, _, idx, meta, _ = model(x, None, epoch_prog)
            ...
            if ep > 0 and ep % CONFIG["sleep_every"] == 0:
                ...
                if dream:
                    for _, _, dx, dy in dream:
                        opt.zero_grad()
                        dl, ds, _, _, dm, _ = model(dx.to(DEVICE), None, epoch_prog)
            ...
            if ep % CONFIG["val_interval"] == 0:
                model.eval()
                with torch.no_grad():
                    vx, vy = get_batch(val_data, CONFIG["batch_size"], CONFIG["seq_len"])
                    vl, vs, _, _, vm, _ = model(vx, None, epoch_prog)
                    v_ce = F.cross_entropy(vl.reshape(-1, CONFIG["vocab_size"]), vy.reshape(-1)).item()
                model.train()
                
                if v_ce < best_val_loss:
                    best_val_loss = v_ce
                    patience_ctr = 0
                    # [NEW] Checkpointing
                    torch.save(model.state_dict(), "sacrsn_checkpoint.pt")
                else:
                    patience_ctr += 1
            ...
            if ep % 20 == 0:
                print(f"Ep {ep:03d} | Loss: {loss.item():.3f} | ValCE: {v_ce if ep%CONFIG['val_interval']==0 else '...'} | "
