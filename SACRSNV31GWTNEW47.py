...
def calculate_loss(logits: torch.Tensor, y: torch.Tensor,
    ...
    struct_loss = struct_scale * (
        ...
        # [M-01 FIX] Subtract gate_ent → penalises HIGH entropy (diffuse gates).
        # High entropy = uniform = no winner = GWT collapses to mean field.
        # Original `+ gate_sparsity_weight * gate_ent` was mathematically inverted.
...
def main():
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            # [E-04 FIX] NaN gradient guard — skip opt.step() to avoid
            # corrupting model weights. Previous code only printed a warning
            # and continued, permanently corrupting parameters.
            nan_in_grads = any(
                p.grad is not None and torch.isnan(p.grad).any()
                for p in model.parameters()
            )
            if nan_in_grads:
                print(f"[!] NaN gradients at epoch {epoch} — skipping opt.step().")
                opt.zero_grad()
                scheduler.step()
                continue
