"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v79 Gradient-Safe)                  ║
...
print(f"--- SACRSN v79: GRADIENT-SAFE EDITION ---")
...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Safe Algebraic Phase: z / |z|
        # Avoids unstable angle() gradients at zero
        z_unit = z / (mag + 1e-9)
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        ...
        # [FIX] Additive Update (Gradient Safe)
        # Old: mem = (1-u)*mem + u*new
        # New: mem = mem + u*(new - mem) -- Equivalent but safer broadcasting
...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        # [FIX] Added jitter to avoid angle() on zero vector
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Nan-to-num safety clamp
            cand = torch.nan_to_num(cand, nan=0.0, posinf=1.0, neginf=-1.0)
            cand = torch.complex(torch.tanh(cand.real), torch.tanh(cand.imag))
            
            cand_safe = cand + 1e-9
            mag = cand_safe.abs()
            valid = mag > 1e-6
            # [FIX] Detach prev_angle to stop BPTT through time for reg target
            prev_angle_detached = prev_angle.detach()
            safe_angle = torch.where(valid, cand_safe.angle(), prev_angle_detached)
            
            diff = (safe_angle - prev_angle_detached).abs()
            diff = torch.min(diff, 2*math.pi - diff)
            
            mask_v = active.view(-1)
            # [FIX] Reshape all terms to 1D to prevent broadcast explosion
            diff_flat = diff.mean(-1).view(-1)
            valid_flat = valid.float().mean(-1).view(-1)
            accum['phase'] += mask_v * diff_flat * valid_flat
            ...
            # [FIX] Additive Update Strategy (Gradient Safe)
            # gw_new = gw + active * (cand - gw)
            # This ensures gradients from 'cand' are multiplied by 0 if active=0
            active_c = torch.complex(active, torch.zeros_like(active))
            delta_gw = active_c * (cand - gw_state)
            gw_state = gw_state + delta_gw
            
            # [FIX] Additive Memory Update
            mem_mask = active.unsqueeze(-1) # (B, 1, 1) real
            # Expand to (B, S, D)
            mem_mask = torch.complex(mem_mask, torch.zeros_like(mem_mask))
            delta_mem = mem_mask * (c_mem - mem_state)
            mem_state = mem_state + delta_mem
