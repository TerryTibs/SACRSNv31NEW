...
print(f"--- SACRSN v69: DIMENSIONALITY CORRECTED ---")
...
class SACRSN_v69(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            w_unc = meta_values[:, 3:4]
            # [CRITICAL FIX] Removed .unsqueeze(-1) which caused broadcasting explosion
            # w_unc is [B, 1], shadow_vec is [B, D]. They broadcast correctly.
            ...
            # Dimensionality assertions to prevent regression
            assert cand_state.dim() == 2, f"State explosion detected: {cand_state.shape}"
            ...
            # Safe Accumulation using view(-1)
            mask_vec = active_mask.view(-1)
            accum['phase'] += mask_vec * diff.mean(dim=-1).view(-1) * valid_frac.view(-1)
...
def main():
    ...
    model = SACRSN_v69(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
