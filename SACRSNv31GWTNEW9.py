...
CONFIG = {
    ...
    "mem_slots": 32,
    "vocab_size": None,     # Auto-set by Tokenizer
    
    # Cognitive Dynamics
    "max_recursion": 8,     # Max "thinking" steps per token
    "halt_bias_init": 3.0,
    ...
    "grad_clip": 1.0,
    "replay_size": 200,     # Hippocampus Size
    "active_plasticity": True,
    "surprise_decay": 0.9   # EMA decay for dopamine
}

print(f"--- SACRSN v46: DIAGNOSTIC EDITION ---")
print(f"Device: {DEVICE}")

# ==========================================
# 1. DATA LOADING & ROBUST BPE
# ==========================================
def load_training_data(filepath="data.txt"):
    # The philosophical text from your earlier scripts
...
class SACRSN_v46(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # Dynamic Memory Update inside loop
        ...
        return gw_state, mem_state, accum, final_indices, avg_batch_ent
    ...
    def forward(self, x_seq, mem_state=None):
        ...
        # Return mem_state for Generation contract
...
def visualize_suite(model, history, transitions):
    ...
    # Loss & Ponder
    plt.subplot(1, 3, 1)
    plt.plot(history['loss'], label='Loss', linewidth=1.5)
    plt.plot(history['ponder'], label='Ponder Cost', linewidth=1.5, alpha=0.7)
    ...
    with torch.no_grad(): _, _, _, _ = model(dummy)
    w = model.decoder.weight.detach().cpu().numpy()
    plt.subplot(1, 3, 2)
    plt.scatter(w[:, :64].mean(0), w[:, 64:].mean(0), alpha=0.6, c='purple', s=10)
...
def extract_logic_rules(transitions, tokenizer):
    print("\n--- EXTRACTED LOGIC RULES (VQ Transitions) ---")
...
def main():
    ...
    model = SACRSN_v46(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    opt = torch.optim.AdamW(model.parameters(), lr=CONFIG["lr"])
    # [NEW] Scheduler for visible LR changes
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if epoch % 20 == 0:
                # [NEW] Enhanced Logging: PPX, Plast, LR
                current_lr = opt.param_groups[0]['lr']
                ppx = math.exp(stats['ent'].item())
                print(f"Ep {epoch:03d} | Loss: {loss.item():.4f} | PPX: {ppx:.1f} | Ponder: {stats['ponder'].item():.3f} | Plast: {plast:.2f} | LR: {current_lr:.1e}")
