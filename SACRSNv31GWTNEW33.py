...
print(f"--- SACRSN v65: STABLE FOUNDATION ---")
...
class StableComplexNorm(nn.Module):
    ...
    def forward(self, z):
        # [FIX] Added eps to prevent NaN on zero vectors
        real = self.norm_r(z.real + 1e-6)
        imag = self.norm_i(z.imag + 1e-6)
        return torch.complex(real, imag)
...
class EMA_VQ(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Clamp distance to prevent negative sqrt NaN
...
def main():
    ...
    model = SACRSN_v65(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if CONFIG["active_plasticity"]:
                ...
                # [FIX] Capped plasticity to 2.0 max to prevent early explosion
