...
print(f"--- SACRSN v67: ROBUST BROADCASTING EDITION ---")
...
class SACRSN_v67(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Reshape active_mask to [B] for accumulation
            # This prevents [16, 1] * [16] -> [16, 16] broadcasting error
            mask_vec = active_mask.view(-1)
            accum['phase'] += mask_vec * diff.mean(dim=-1) * valid_frac
            ...
            accum['vq'] += (mask_vec * vq_loss_sample)
    ...
    with torch.no_grad():
        try:
            ...
            for i in range(length):
                ...
                # Replace NaNs in probs just in case
...
def main():
    ...
    model = SACRSN_v67(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
