...
print(f"--- SACRSN v72: EXECUTION FIXED EDITION ---")
...
class SACRSN_v72(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            # Standard Kaiming/Xavier for complex path stability
            torch.nn.init.xavier_uniform_(m.weight)
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)

    # [FIXED] Updated step_modules to return ALL values
    def _step_modules(self, curr_state, mem_state):
        v_out, vq_loss_sample, indices, norm_entropy = self.vq(curr_state)
        ...
        batch_ent = norm_entropy
        
        # Engineering safety assertion
        assert v_out.shape == curr_state.shape, "VQ shape mismatch"
...
def main():
    ...
    model = SACRSN_v72(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if CONFIG["active_plasticity"]:
                ...
                plast = min(2.0, 1.0 + avg_surprise*0.5)
