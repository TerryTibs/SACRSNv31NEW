...
try:
    # Attempt strict determinism, warn if unavailable
...
print(f"--- SACRSN v68: SHAPE-SAFE EDITION ---")
...
class StableComplexNorm(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # [FIX] Epsilon in constructor for safety
...
class SACRSN_v68(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        B = x_t.size(0)
        # Ensure active_mask is [B, 1] for broadcasting against memory [B, S, D]
        active_mask = torch.ones(B, 1, device=DEVICE)
        
        # Accumulators use 1D vectors [B] to avoid broadcasting ambiguity
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # Use scalar sum check for active_mask (safe)
            ...
            w_unc = meta_values[:, 3:4]
            dynamic_shadow_weight = CONFIG["shadow_weight"] * w_unc.unsqueeze(-1) # Fixed config key name
            ...
            # [FIX] Forced Flattening: Ensure all terms are [B] before adding to accumulator
            mask_flat = active_mask.view(-1)
            diff_flat = diff.mean(dim=-1).view(-1)
            valid_flat = valid_frac.view(-1)
            accum['phase'] += mask_flat * diff_flat * valid_flat
            ...
            accum['ponder'] += mask_flat * CONFIG["ponder_cost"]
            total_steps_batch += mask_flat
            accum['vq'] += (mask_flat * vq_loss_sample.view(-1))
            
            # Update indices for active samples
            still_active = mask_flat > 0.5
...
def main():
    ...
    model = SACRSN_v68(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
