...
class SACRSN_Unified(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor, mem_state=None):
        ...
        for t in range(T):
            x_t = gw_seq[:, t]
            # Detach mem_state between tokens: memory is already detached inside
            # AssociativeMemory.forward(), but this double-guard prevents any
            # gradient leaking through the mem_mask_c update path in forward_step.
