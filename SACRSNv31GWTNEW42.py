...
try:
    ...
    print("Warning: Strict determinism not available on this hardware.")
...
print(f"--- SACRSN v74: DIAMOND EDITION ---")
...
class EMA_VQ(nn.Module):
    def __init__(self, dim, num_concepts):
        ...
        # [FIX] Freeze embedding from optimizer (pure EMA update)
    ...
    def forward(self, z):
        ...
        if self.training:
            with torch.no_grad():
                ...
                # [FIX 2] Robust VQ Cluster Normalization
                cluster_size = self.ema_cluster_size + self.epsilon
                # Force minimum divisor to 1.0 to preserve magnitude of dead codes
                cluster_size = cluster_size + (cluster_size < 1.0).float()
                
                dw = torch.matmul(encodings.t(), z_flat.detach())
                self.ema_w.mul_(self.decay).add_(dw, alpha=1 - self.decay)
                
                new_weight = self.ema_w / cluster_size.unsqueeze(1)
                new_weight = torch.clamp(new_weight, -5.0, 5.0)
...
class HebbianSensory(nn.Module):
    ...
    def forward(self, gw_state):
        ...
        vis_scores = vis_scores - vis_scores.max(dim=-1, keepdim=True)[0].detach()
        ...
        aud_scores = aud_scores - aud_scores.max(dim=-1, keepdim=True)[0].detach()
...
class SACRSN_v74(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Safe Shadow Normalization (Audit Fix 1)
            shadow_sum = shadow_probs.sum(dim=1, keepdim=True)
            shadow_norm = torch.where(
                shadow_sum > 1e-6,
                shadow_probs / shadow_sum,
                torch.ones_like(shadow_probs) / shadow_probs.size(1) # Default to uniform if sum is 0
            )
            ...
            w_unc = meta_values[:, 3:4]
            dynamic_shadow_weight = CONFIG["shadow_weight"] * w_unc
            
            update = winner_vec + (dynamic_shadow_weight * shadow_vec)
            
            update = torch.complex(torch.tanh(update.real), torch.tanh(update.imag))
            cand_state = 0.6 * curr_state + 0.4 * update
            cand_state = torch.complex(torch.tanh(cand_state.real), torch.tanh(cand_state.imag))
            ...
            mask_vec = active_mask.view(-1)
            diff_flat = diff.mean(dim=-1).view(-1)
            valid_flat = valid_frac.view(-1)
            accum['phase'] += mask_vec * diff_flat * valid_flat
            ...
            total_steps_batch += mask_vec
            accum['vq'] += (mask_vec * vq_loss_sample.view(-1))
        ...
        div = torch.max(total_steps_batch, torch.ones_like(total_steps_batch))
        accum['vq'] = accum['vq'] / div
...
def visualize_suite(model, history, transitions, meta_history):
    ...
    if history['grad_norms']:
        ...
        plt.title("Gradient Norms"); plt.legend(fontsize=6); plt.grid(True, alpha=0.3)
        ...
        try:
            ...
            for i in range(length):
                ...
                if torch.isnan(logits).any():
                    print("\n[!] WARNING: Model generated NaN logits.")
                    break
                ...
                # Safe softmax
                logits_safe = logits[:, -1, :] / temp
                logits_safe = logits_safe - logits_safe.max(dim=-1, keepdim=True)[0]
...
# 6. MAIN EXECUTION
# ==========================================
def calculate_loss(logits, y, stats, meta_vals, plasticity=1.0, epoch=0, total_epochs=1):
    ...
    if isinstance(stats.get('depth_per_sample'), torch.Tensor):
        ...
        depth_penalty = CONFIG['depth_target_weight'] * (under + over).mean()
    else:
        depth_penalty = 0.0
    
    progress = epoch / total_epochs
    current_ent_weight = CONFIG['entropy_weight_start'] * (1 - progress) + \
    ...
    struct_loss = struct_scale * (
        ...
        (current_ent_weight * (1.0 + w_div)) * stats['ent'] +
...
def main():
    ...
    model = SACRSN_v74(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            with torch.no_grad():
                model.arbitrator.weight.mul_(1.0 - CONFIG["synaptic_decay"])
                model.meta_head.weight.mul_(1.0 - CONFIG["synaptic_decay"])
                model.decoder.weight.mul_(1.0 - CONFIG["synaptic_decay"])
            ...
            if CONFIG["active_plasticity"]:
                ...
                # [FIX] Capped plasticity to 1.5 for extra safety against gradient explosion
                plast = min(1.5, 1.0 + avg_surprise*0.5)
            
            _, final_loss = calculate_loss(logits, y, stats, meta_vals, plasticity=plast, epoch=epoch, total_epochs=CONFIG['epochs'])
            
            if torch.isnan(final_loss):
                print(f"[!] Warning: NaN loss detected at Ep {epoch}. Skipping step.")
                opt.zero_grad()
                continue
            ...
            # [FIX] Deep NaN Scanner for diagnostics
            if torch.isnan(model.decoder.weight.grad).any():
                print(f"[!] Alert: NaN gradients detected in Decoder at Ep {epoch}")
            ...
            if epoch > 0 and epoch % 50 == 0:
                ...
                if dream:
                    for _, dx, dy in dream:
                        ...
                        _, d_loss = calculate_loss(d_logits, dy, d_stats, d_meta, plasticity=1.0, epoch=epoch, total_epochs=CONFIG['epochs'])
            ...
            if epoch % 20 == 0:
                ...
                text_ppx = math.exp(min(100, loss_ce_raw.item()))
