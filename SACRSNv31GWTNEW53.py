"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v77 Ultimate Restoration)           ║
...
print(f"--- SACRSN v77: ULTIMATE RESTORATION ---")
...
class QuadratureModule(nn.Module):
    ...
    def forward(self, gw_state):
        z = self.net(gw_state)
        # 90-degree phase shift (Oppositional Thinking)
...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIXED] Strict broadcasting safety for phase accumulation
            ...
            # [FIXED] Memory mask broadcasting
...
# [RESTORED] Full Verbose Generation
def generate_text(model, tokenizer, seed="The", length=60, temp=0.8, verbose=False):
    print(f"\n── Generating ('{seed}', T={temp}) ──")
    model.eval()
...
# [RESTORED] Logic Rules
def extract_logic_rules(transitions):
    print("\n── Extracted Rules ──")
    print(f"{'FROM':<6} -> {'TO':<6} | {'COUNT':<6}")
    for (u, v), c in Counter(transitions).most_common(10):
        print(f"{u:<6} -> {v:<6} | {c:<6}")

# [RESTORED] Anomaly Detector
...
def visualize_suite(history, transitions, meta_history):
    ...
    plt.plot(history['ponder'], label='Ponder')
    plt.title("Cognitive Effort"); plt.legend()
    
    # 2. Depth
    plt.subplot(2, 3, 2)
    plt.plot(history['depth'], color='purple', label='Depth')
    plt.axhline(CONFIG['target_depth'], color='red', linestyle='--')
    plt.title("Recursion Depth"); plt.legend()
    ...
    plt.title("Meta-Values"); plt.legend()
    ...
    if history['grads']:
        ...
        for k in keys:
            plt.plot([x[k] for x in history['grads']], label=k)
    plt.title("Gradient Norms"); plt.legend(fontsize=6)
