...
CONFIG = {
    ...
    "max_recursion":     5,
    "halt_bias_init":    6.0,   # FIXED from 3.0. The halt equation is:
                                 #   halt_prob = σ(softplus(bias) - softplus(scale) × norm_loss)
                                 # At norm_loss=1.0 (VQ loss ≈ baseline):
                                 #   bias=3.0, scale=10: σ(3.13 - 10 × 1) = σ(-6.87) = 0.001 → depth=5.0
                                 #   bias=6.0, scale=4.0: σ(6.02 - 4.02 × 1) = σ(2.0) = 0.88 → depth=1.1
                                 # bias=6, scale=4 gives median halt at norm_loss≈1.5, gentle curve.
    "halt_scale_init":   4.0,   # FIXED from 10.0. Shallower sigmoid → halt responds proportionally
                                 # to VQ quality instead of being a near-binary switch.
    "ponder_cost":       0.001, # FIXED from 0.01. At depth=5 and seq=128:
                                 # ponder = 0.01×5 = 0.05 accumulated → adds ~0.5% to total loss.
                                 # Not catastrophic, but with halt broken it was an unearned penalty.
                                 # 0.001 keeps the effort signal without fighting CE.
    ...
    # ── Training ──────────────────────────────────────────────────────────
    "epochs":       2000,
    "lr":           1e-4,   # FIXED from 5e-5. NaN is now fully resolved; gradient
                             # norm is stable at 0.3-0.35 which is too small to move
                             # weights meaningfully at 5e-5. 1e-4 doubles the step size
                             # and is safe given grad_clip=0.5 as the backstop.
    "replay_size":  200,
