...
CONFIG = {
    ...
    # [RESTORED] The Shadow (Subconscious Influence)
    "shadow_weight": 0.3,
    "critic_weight": 0.1,
...
print(f"--- SACRSN v60: UNIFIED CONSCIOUSNESS ---")
...
class ComplexLinear(nn.Module):
    def __init__(self, in_features, out_features):
        super().__init__()
        # Independent matrices for maximum expressivity
...
def main():
    ...
    model = SACRSN_v60(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if epoch % 20 == 0:
                ...
                print(f"Ep {epoch:03d} | Loss: {final_loss.item():.3f} | TextPPX: {text_ppx:.1f} | CodePPX: {code_ppx:.1f} | Depth: {stats['depth'].item():.2f}/{CONFIG['target_depth']:.1f} | Plast: {plast:.2f}")
                if history['grad_norms']:
