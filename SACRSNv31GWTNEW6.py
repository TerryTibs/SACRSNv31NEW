...
# ==========================================
# 0. STRICT DETERMINISM (Audit Fix 1)
# ==========================================
SEED = 42
...
# [FIX] Explicit deterministic algorithms as requested
try:
    torch.use_deterministic_algorithms(True, warn_only=True)
except:
    print("Warning: Hardware does not support strict deterministic algorithms. Proceeding.")
...
CONFIG = {
    ...
    "surprise_decay": 0.9   # [FIX] EMA decay for plasticity stability
}

print(f"--- SACRSN v43: RECTIFIED ---")
...
class AssociativeMemory(nn.Module):
    # [FIX] Renamed to reflect Audit Fix 4 (Associative instead of Sliding Tape)
    def __init__(self, dim, slots):
        super().__init__()
        self.dim, self.slots = dim, slots
        self.gate_net = nn.Linear(dim * 2, 1)
        # [FIX] Added Address Network for Soft Write
    ...
    def forward(self, gw_state, prev_mem):
        ...
        # 2. Write Gate & Address
        flat_input = torch.cat([gw_state.real, gw_state.imag], dim=-1)
        write_gate = torch.sigmoid(self.gate_net(flat_input)).unsqueeze(1) # (B, 1, 1)
        
        # [FIX] Learned Addressing (Soft Write) instead of Roll
        # The model predicts a distribution over slots to update
        write_weights = F.softmax(self.address_net(flat_input), dim=-1).unsqueeze(-1) # (B, Slots, 1)
        
        new_slot = gw_state.unsqueeze(1) # (B, 1, Dim)
        
        # Update: Memory = (1 - gate*weight) * Old + (gate*weight) * New
        # This allows updating specific slots without shifting everything
...
class EntropyRegularizedVQ(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Optimized Distance (Audit Fix 3)
        # |x-y|^2 = |x|^2 + |y|^2 - 2xy
        # Avoids O(N^3) overhead of cdist
        ...
        # [FIX] Batch-level Entropy (Audit Fix 2)
        # Compute distribution over the WHOLE batch to prevent collapse
        # We return a single scalar for the batch
        encodings = F.one_hot(indices, num_classes=self.embedding.num_embeddings).float()
        avg_probs = encodings.mean(0) # Average over batch
...
class SACRSN_v43(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        # Entropy is batch-level, tracked separately
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # VQ returns batch-level entropy scalar
            v_out, vq_loss_sample, indices, batch_ent = self.vq(curr_state)
            
            # We add entropy pressure if ANY sample is still thinking
            # (Approximation: we want codebook diversity generally)
        ...
        # Average entropy over steps taken (approx)
        avg_batch_ent = total_batch_entropy / CONFIG["max_recursion"]
...
def visualize_suite(model, history, transitions):
    ...
    plt.title("Semantic Associations")
    plt.tight_layout()
    plt.show()

def extract_logic_rules(transitions, tokenizer):
    print("\n--- EXTRACTED LOGIC RULES ---")
    counts = Counter(transitions)
...
def anomaly_detector(model, tokenizer):
    ...
    def get_surprise(text):
        ...
        with torch.no_grad():
            ...
            # Use raw logits to get score for target tokens
...
def main():
    ...
    # [FIX] EMA for Plasticity Stability (Audit Fix 5)
    avg_surprise = 2.0 
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            # [FIX] Stabilized Plasticity Calculation
            plasticity = 1.0
            if CONFIG["active_plasticity"]:
                current_surprise = loss_ce.item()
                # Update moving average
                avg_surprise = (CONFIG["surprise_decay"] * avg_surprise) + \
                               ((1 - CONFIG["surprise_decay"]) * current_surprise)
                # Use smoothed surprise for modulation
                plasticity = min(3.0, 1.0 + (avg_surprise * 0.5))
