"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v83 Decoupled Gradient)             ║
...
print(f"--- SACRSN v83: DECOUPLED GRADIENT EDITION ---")
...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z):
        mag = torch.abs(z)
        # [FIX] Clamp magnitude to avoid div-by-zero gradient explosion
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        ...
        # Additive Update with Tanh clamp
...
class SACRSN_v83(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Decoupled Gradient Update (Component-wise masking)
            # active_detach blocks gradient flow from State -> Halt -> State feedback loop
            active_detach = active.detach().squeeze(-1) # [B]
            
            gw_real = gw_state.real * (1.0 - active_detach) + cand.real * active_detach
            gw_imag = gw_state.imag * (1.0 - active_detach) + cand.imag * active_detach
            gw_state = torch.complex(gw_real, gw_imag)
            
            mem_real = mem_state.real * (1.0 - active_detach.unsqueeze(-1)) + c_mem.real * active_detach.unsqueeze(-1)
            mem_imag = mem_state.imag * (1.0 - active_detach.unsqueeze(-1)) + c_mem.imag * active_detach.unsqueeze(-1)
...
def main():
    ...
    model = SACRSN_v83(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
