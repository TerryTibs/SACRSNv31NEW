...
class SACRSN_Unified(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int):
        ...
        # Improvement 9 — halt temperature: starts at 0.5 (biases toward deep
        # exploration), anneals to 1.0 by step 500 (normal operation).
        # Multiplied into halt_prob in _halt(); trained away in the loop.
        ...
        # decoder.weight shape: (vocab, dim*2)  [Linear stores transposed]
        # Tying halves I/O parameter cost (~2.1M → ~1M for vocab=4224)
        # and regularises the representation space — the encoder and decoder
        # learn a single shared embedding manifold.
    ...
    def forward_step(self, x_t: torch.Tensor, gw_state: torch.Tensor,
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            identity   = torch.eye(5, device=DEVICE).unsqueeze(0)              # (1,5,5)
            orth_loss  = (gram - identity).pow(2).mean(dim=(-1, -2))           # (B,)  /25
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if model.halt_temp.item() < 1.0:
                model.halt_temp.clamp_(0.0, 1.0).add_(0.001)
