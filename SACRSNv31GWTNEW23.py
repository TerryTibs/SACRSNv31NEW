...
print(f"--- SACRSN v56: HARDENED DIALECTIC EDITION ---")
...
class SACRSN_v55(nn.Module):
    ...
    def _step_arbitration(self, curr_state, meta_values):
        ...
        raw_gates = F.softmax(self.arbitrator(flat), dim=-1)
        
        # [FIX] Entropy floor: blend toward uniform to prevent total winner-take-all
        # w_uncertainty (meta_values[:,3]) gates how much we enforce entropy floor
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        # Meta-Values computed once per step (before recursion loop)
        flat_gw = torch.cat([gw_state.real, gw_state.imag], dim=-1)
        meta_values = torch.sigmoid(self.meta_head(flat_gw))  # [B, 4]: Ethics, Diversity, Ponder, Uncertainty
        
        # [FIX] Wire Ethics meta-value → scale critic contribution
        # Higher ethics = stronger internal dialectic challenge
        w_ethics = meta_values[:, 0:1]  # [B, 1]
        critic_scale = CONFIG["critic_weight"] + w_ethics * (1.0 - CONFIG["critic_weight"])
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Use active_any to avoid repeated GPU→CPU syncs
            ...
            # [REFACTORED] Arbitration with entropy floor
            gates, gate_ent = self._step_arbitration(curr_state, meta_values)
            module_wins += (gates * active_mask)
            if active_any:
                total_gate_entropy += gate_ent
            
            # [FIX] Apply ethics-gated critic scaling to stream 4 (Critic)
            # This connects Ethics meta-value to actual structural effect
            gates_scaled = gates.clone()
            gates_scaled[:, 4:5] = gates[:, 4:5] * critic_scale
            
            # Complex Mixing of 5 Streams: Gate, Mem, VQ, Sensory, Critic
            update_real = (gates_scaled[:,0:1]*g_out.real + gates_scaled[:,1:2]*m_out.real + 
                           gates_scaled[:,2:3]*v_out.real + gates_scaled[:,3:4]*s_out.real +
                           gates_scaled[:,4:5]*c_out.real)
            update_imag = (gates_scaled[:,0:1]*g_out.imag + gates_scaled[:,1:2]*m_out.imag + 
                           gates_scaled[:,2:3]*v_out.imag + gates_scaled[:,3:4]*s_out.imag +
                           gates_scaled[:,4:5]*c_out.imag)
        ...
        return gw_state, mem_state, accum, final_indices, avg_batch_ent, avg_gate_ent, avg_slot_ent, avg_depth, meta_values, module_wins, avg_sensory_ent
...
class HippocampalBuffer:
    def __init__(self, capacity):
        ...
        self.buffer = []
        # [FIX] Track running stats for loss normalization
        # Storing raw CE avoids plasticity-scale drift making early memories permanent
        self._loss_sum = 0.0
        self._loss_count = 0

    def add(self, x, y, raw_ce_loss):
        """Store based on raw CE loss (intrinsic surprise), not plasticity-scaled loss."""
        # [FIX] Use z-score normalized loss for fair comparison across training
        self._loss_sum += raw_ce_loss
        self._loss_count += 1
        running_mean = self._loss_sum / self._loss_count
        # Priority = how surprising relative to average (positive = above average surprise)
        priority = raw_ce_loss - running_mean
...
def calculate_loss(logits, y, stats, meta_vals, plasticity=1.0):
    ...
    # meta_vals: [Ethics(→critic_scale), Diversity(→entropy_weight), Ponder(→ponder_cost), Uncertainty(→halt_resist)]
    w_ethics, w_div, w_ponder, w_unc = meta_vals.mean(0)
    
    # [FIX] Depth target regularization: prevent collapse to minimal/maximal recursion
