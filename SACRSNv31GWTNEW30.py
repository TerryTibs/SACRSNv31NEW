...
# [FIX] Enforce strict determinism (Audit Fix 1)
try:
    torch.use_deterministic_algorithms(True)
except Exception as e:
    print(f"Warning: Deterministic algorithms not available: {e}")
...
CONFIG = {
    ...
    # Decay (Applies to Arbitrator/Decoder)
    "synaptic_decay": 1e-5, 
    ...
    # [TUNED] Stronger Depth Targeting (Audit Fix 5)
    # 10x stronger penalty to prevent "Decorated LM" collapse
    "target_depth": 4.0,
    "depth_target_weight": 0.05, 
}

print(f"--- SACRSN v62: THE FINAL ARTIFACT ---")
...
def main():
    ...
    model = SACRSN_v62(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
