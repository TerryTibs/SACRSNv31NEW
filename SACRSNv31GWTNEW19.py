...
try:
    # [FIX] Set warn_only=False for strict research reproducibility if supported
...
CONFIG = {
    ...
    "vq_decay": 0.99,       # EMA Decay for Codebook
    
    # Regularization
    "entropy_weight": 0.05, 
    "gate_sparsity_weight": 0.01,
    "slot_balance_weight": 0.01, # [NEW] Prevent memory slot collapse
    "phase_reg": 0.01,
...
print(f"--- SACRSN v53: STABILIZED COGNITIVE ENGINE ---")
print(f"Device: {DEVICE}")

# ==========================================
# 1. DATA & BPE (Optimized)
# ==========================================
...
class RobustBPE:
    ...
    def encode(self, text):
        ...
        # [FIX] Add BOS and EOS tokens for sequence boundary learning
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        ...
        # [FIX] Address Weights Entropy (Prevent Slot Collapse)
        logits = self.address_net(flat_input)
        write_weights = F.softmax(logits, dim=-1) # (B, Slots)
        
        # Calculate entropy of write distribution for regularization
...
class EMA_VQ(nn.Module):
    # [FIX] True EMA VQ-VAE Implementation (Audit Fix 2)
    ...
    def forward(self, z):
        ...
        # [FIX] EMA Update during training
        if self.training:
...
class SACRSN_v53(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        total_slot_entropy = 0 # [FIX] Track memory slot usage
...
def visualize_suite(model, history, transitions):
    ...
    # [FIX] Limited Deque for visualization
    plt.subplot(1, 3, 3)
    G = nx.DiGraph()
...
def calculate_loss(logits, y, stats, plasticity=1.0):
    # [FIX] Plasticity applied ONLY to Cross-Entropy (Audit Fix 4)
    loss_ce = F.cross_entropy(logits.reshape(-1, CONFIG["vocab_size"]), y.reshape(-1))
    
    # Structural losses (not scaled by dopamine)
    struct_loss = 0.1 * stats['vq'] + \
                  ...
                  CONFIG['slot_balance_weight'] * stats['slot_ent'] # [FIX] Slot Entropy
...
def main():
    ...
    model = SACRSN_v53(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if CONFIG["active_plasticity"]:
                # Calculate raw CE for surprise tracking
                with torch.no_grad():
                    raw_ce = F.cross_entropy(logits.reshape(-1, CONFIG["vocab_size"]), y.reshape(-1))
                avg_surprise = CONFIG["surprise_decay"]*avg_surprise + (1-CONFIG["surprise_decay"])*raw_ce.item()
                plast = min(3.0, 1.0 + avg_surprise*0.5)
            
            loss_ce, final_loss = calculate_loss(logits, y, stats, plasticity=plast)
            ...
            if epoch > 0 and epoch % 50 == 0:
                ...
                if dream:
                    for _, dx, dy in dream:
                        ...
                        # Replay full structural objective
