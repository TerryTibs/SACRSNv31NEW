...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # .angle() backward = atan2 gradient = ±component / (x²+y²).
        # At near-zero magnitude (x²+y²→0) this gradient → ±∞ → NaN.
        # Phase is a steering direction for the output, not a source of
        # gradient for the normalisation itself — detach is correct here.
