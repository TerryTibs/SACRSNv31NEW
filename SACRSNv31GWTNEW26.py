...
print(f"--- SACRSN v59: PRODUCTION-READY DIALECTIC EDITION ---")
...
class RobustBPE:
    ...
    def train(self, text):
        ...
        # [PERF] Precompute set of all known merge pairs for O(1) existence check,
        # and build a sorted list for fast O(k log k) best-pair search during encode.
        # This avoids O(n*k) linear scan per word during large-corpus encoding.
    ...
    def encode(self, text):
        words = re.findall(r"[\w']+|[^\s\w]", text)
        ids = []
        # [PERF] Use precomputed merge set for O(1) existence check.
        # Falls back to full dict if _merge_set not yet built (e.g. loaded from checkpoint).
        ...
        for word in words:
            ...
            while len(word_tokens) > 1:
                ...
                for i in range(len(word_tokens)-1):
                    ...
                    # [PERF] Fast set membership check before dict rank lookup
...
class SACRSN_v55(nn.Module):
    ...
    def _step_halting(self, vq_loss_sample, meta_values):
        ...
        # [FIX] Stochastic halting during training: sample from halt_prob rather than
        # hard-thresholding at 0.5. Smooths gradient boundary; reduces depth jitter near threshold.
        # Straight-through estimator preserves gradients. Deterministic at inference.
        if self.training:
...
def generate_text(model, tokenizer, seed="The", length=50, temp=0.8, verbose_modules=False):
    """Generate text with temperature sampling and stream-of-consciousness module logging.
    
    Args:
        verbose_modules: If True, print dominant module for every token (not just first 5).
                        Useful for analyzing module behavior; False by default to reduce spam.
    """
    ...
    if verbose_modules:
        print("Stream of Consciousness (all tokens):")
    else:
        print("Stream of Consciousness (first 5 tokens):")
    ...
    with torch.no_grad():
        ...
        for i in range(length):
            ...
            # [FIX] verbose_modules=True logs every token; default only logs first 5
            if verbose_modules or i < 5:
                winner_idx = torch.argmax(wins[0]).item()
                # Also show runner-up for richer insight when verbose
                if verbose_modules:
                    top2 = torch.topk(wins[0], min(2, wins.size(-1)))
                    dominant = modules[top2.indices[0].item()]
                    runner = modules[top2.indices[1].item()] if wins.size(-1) > 1 else "—"
                    print(f"\nToken {i}: [{dominant}] > [{runner}]", end="  ")
                else:
...
def dream_graph_walk(model, transitions, tokenizer, steps=20, temp=1.0):
    """Weighted associative dream walk over the concept transition graph.
    
    Args:
        temp: Temperature for transition probabilities. <1.0 = more deterministic (follow strong paths),
              >1.0 = more exploratory. Probabilities re-normalized at each step.
    """
    ...
    # [FIX] Build frequency-weighted transition map from Counter
    transition_counts = Counter(transitions)
    graph = defaultdict(dict)  # graph[u][v] = count
    for (u, v), count in transition_counts.items():
        graph[u][v] = count
    
    # Pick random start, weighted by out-degree (concepts with more transitions preferred)
    ...
    for step in range(steps):
        ...
        # [FIX] Dynamic re-normalization each step with temperature scaling.
        # This prevents probability collapse on very long walks where one path
        # accumulates extreme counts relative to others.
...
def log_grad_norms(model):
    """Per-module gradient norm logging for visibility into gradient dominance.
    
    Note: Modules with frozen (requires_grad=False) or unused parameters will show 0.0.
    A norm of 0.0 may mean: (a) frozen params, (b) module not in computation graph this
    batch, or (c) gradients genuinely zero. Check model.named_parameters() for (a).
    """
    module_groups = {
    ...
    for name, module in module_groups.items():
        ...
        for p in module.parameters():
            if p.requires_grad:
                ...
                n_frozen += 1
        norm = total_norm ** 0.5 if n_with_grad > 0 else 0.0
        # Annotate frozen modules so zero doesn't mislead
        norms[name] = norm if n_frozen == 0 else f"{norm:.4f}(+{n_frozen}frozen)"
    return norms
