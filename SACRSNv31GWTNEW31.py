...
CONFIG = {
    ...
    # Training
    "epochs": 300,
    # [FIX] Lower LR to prevent complex-value divergence
    "lr": 3e-4,
    "grad_clip": 1.0,
    "replay_size": 200,
...
print(f"--- SACRSN v63: ROBUST EDITION ---")
...
def generate_text(model, tokenizer, seed="The", length=50, temp=0.8, verbose_modules=False):
    ...
    with torch.no_grad():
        # [FIX] Safe Sampling: Catch errors if weights are broken/NaN
        try:
            ...
            for i in range(length):
                ...
                if torch.isnan(logits).any():
                    print("\n[!] WARNING: Model generated NaN logits. stopping generation.")
                    break
...
def main():
    ...
    model = SACRSN_v63(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            # [FIX] NaN Guard in Training Loop
            ...
            if epoch % 20 == 0:
                ...
                text_ppx = math.exp(min(100, loss_ce_raw.item())) # Clamp ppx display
