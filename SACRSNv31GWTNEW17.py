...
CONFIG = {
    ...
    "mem_slots": 32,
    "vocab_size": None,
    "codebook_size": 128,   # Semantic Abstraction
    ...
    # Regularization
    "entropy_weight": 0.05, 
    "gate_sparsity_weight": 0.01, # [FIX] Force modular decision making
    "phase_reg": 0.01,
...
print(f"--- SACRSN v51: HARDENED RESEARCH EDITION ---")
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        # [FIX] True Hermitian Inner Product for Similarity
        # (B, 1, D) . (B, S, D) -> (B, S)
...
class EntropyRegularizedVQ(nn.Module):
    ...
    def forward(self, z):
        ...
        # [FIX] Configurable Beta for Commitment
        beta = CONFIG["vq_commitment_beta"]
        loss_sample = ((z_q - z_flat.detach())**2).mean(dim=-1) + beta * ((z_q.detach() - z_flat)**2).mean(dim=-1)
        ...
        norm_entropy = batch_entropy / math.log(self.num_concepts)
...
class SACRSN_v51(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [FIX] Adaptive Halting Statistics
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        total_gate_entropy = 0 # [FIX] Gate sparsity tracker
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Update Halting EMA (detached to not break graph)
            if self.training:
                with torch.no_grad():
            ...
            # [FIX] Gate Entropy (Minimizing this encourages sparsity/decision)
            if active_mask.sum() > 0:
            ...
            # [FIX] Adaptive Normalized Halting
            ...
            # [FIX] Optimized Memory Masking (Linear Interp instead of Where)
            mem_mask = active_mask.unsqueeze(-1)
            # Memory = Old + Mask * (New - Old)
        ...
        return gw_state, mem_state, accum, final_indices, avg_batch_ent, avg_gate_ent, avg_depth
    ...
    def forward(self, x_seq, mem_state=None):
        ...
        outputs, all_indices = [], []
        stats = {k: 0 for k in ['vq', 'ponder', 'phase', 'ent', 'gate_ent', 'depth']}
        
        for t in range(T):
            x_t = gw_seq[:, t]
            gw_state, mem_state, step_stats, indices, batch_ent, gate_ent, depth = self.forward_step(x_t, gw_state, mem_state)
...
def visualize_suite(model, history, transitions):
    ...
    with torch.no_grad():
        ...
        # [FIX] Correct thought trace update
        ...
        traj_real, traj_imag = [], []
        for _ in range(20):
            gw_dummy, mem_trace, _, _, _, _, _ = model.forward_step(x_dummy, gw_dummy, mem_trace)
...
def main():
    ...
    model = SACRSN_v51(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            loss = (loss_ce * plast) + \
                   ...
                   CONFIG['gate_sparsity_weight']*stats['gate_ent'] # [FIX] Add Gate Penalty
            ...
            if epoch % 20 == 0:
                ...
                # CodePPX is now e^(norm_ent * log(128)) = e^entropy
                # We un-normalize to get raw entropy exponent
                raw_ent = stats['ent'].item() * math.log(CONFIG['codebook_size'])
                code_ppx = math.exp(raw_ent)
                print(f"Ep {epoch:03d} | Loss: {loss.item():.3f} | TextPPX: {text_ppx:.1f} | CodePPX: {code_ppx:.1f} | Depth: {stats['depth'].item():.2f} | Plast: {plast:.2f}")
