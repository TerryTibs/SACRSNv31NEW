...
CONFIG = {
    ...
    # Deadband Depth Targeting
    "depth_min": 2.0,
    "depth_max": 6.0,
    "depth_target_weight": 0.05, 
}

print(f"--- SACRSN v71: STABILIZED NUMERICS EDITION ---")
...
class AssociativeMemory(nn.Module):
    ...
    def forward(self, gw_state, prev_mem):
        ...
        # [FIX] Logit Stabilization before softmax
    ...
    def forward(self, z):
        ...
        if self.training:
            with torch.no_grad():
                ...
                cluster_size = cluster_size.clamp(min=1.0) # Prevent tiny division
                
                dw = torch.matmul(encodings.t(), z_flat.detach())
                self.ema_w.mul_(self.decay).add_(dw, alpha=1 - self.decay)
                
                new_weight = self.ema_w / cluster_size.unsqueeze(1)
                # Hard Clamp magnitude to prevent explosion
...
def main():
    ...
    model = SACRSN_v71(CONFIG["vocab_size"], CONFIG["embed_dim"]).to(DEVICE)
