"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v76 Windows-Compatible)             ║
...
║  ─────────────────────────────────────────────────────────────────────  ║
║  • 5-module Global Workspace: Gate · Memory · VQ · Sensory · Quadrature ║
║  • Adaptive T.O.T.E. halting (learnable bias + scale on VQ loss EMA)   ║
║  • Hippocampal replay with online z-score priority (Heapq)             ║
║  • Cosine-annealed AdamW, gradient clipping, Strict NaN-guards          ║
...
# Try to enforce deterministic algorithms, warn if hardware prevents it
try:
...
print(f"--- SACRSN v76: WINDOWS-COMPATIBLE EDITION ---")
...
def load_training_data(filepath="data.txt"):
    # [FIX] Enhanced fallback text for immediate testing
    FALLBACK = (
    ...
    if os.path.exists(filepath):
        ...
        try:
            ...
            # [FIX] Fallback to CP1252 (Windows) if UTF-8 fails
...
class RobustBPE:
    ...
    def decode(self, ids):
        ...
        # Restore basic punctuation spacing for readability
...
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z):
        ...
        angle = z.angle()
        return torch.complex(norm_mag * torch.cos(angle), norm_mag * torch.sin(angle))

class StableComplexNorm(nn.Module):
    """Component-wise normalization for internal memory stability."""
    def __init__(self, dim):
        super().__init__()
...
class HermitianGating(nn.Module):
    """Gate = Sigmoid( Real( q * k_conjugate ) )"""
    def __init__(self, dim):
        super().__init__()
...
class QuadratureModule(nn.Module):
    """The Critic: Rotates state 90 degrees in complex plane."""
    def __init__(self, dim):
        super().__init__()
...
class EMA_VQ(nn.Module):
    def __init__(self, dim, num_concepts):
        ...
        self.embedding.weight.requires_grad = False # EMA only
...
class SubmodalityAnchorModule(nn.Module):
    ...
    def forward(self, gw_state):
        ...
        # 90-degree rotation for orthogonality
...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            mag = cand.abs()
            valid = mag > 1e-6
            safe_angle = torch.where(valid, cand.angle(), prev_angle)
            ...
            gw_state = torch.complex(
                torch.where(active > 0.5, cand.real, gw_state.real),
                torch.where(active > 0.5, cand.imag, gw_state.imag)
            )
            
            # Update Memory (Masked)
            mem_mask = torch.complex(active.unsqueeze(-1), torch.zeros_like(active.unsqueeze(-1)))
...
# ═══════════════════════════════════════════════════════════════════════════
# 5.  HIPPOCAMPAL BUFFER
# ═══════════════════════════════════════════════════════════════════════════
class HippocampalBuffer:
    def __init__(self, capacity):
        self.capacity = capacity
        self.buffer, self._cnt = [], 0
...
# ═══════════════════════════════════════════════════════════════════════════
# 6.  DIAGNOSTICS & TRAIN UTILS
# ═══════════════════════════════════════════════════════════════════════════
def log_grad_norms(model):
    groups = {'enc': model.encoder, 'gate': model.gate_mod, 'mem': model.memory_module,
              'vq': model.vq, 'sense': model.sensory_cortex, 'quad': model.quadrature,
              'arb': model.arbitrator, 'meta': model.meta_head, 'dec': model.decoder}
    norms = {}
    for n, m in groups.items():
        t = sum(p.grad.data.norm(2).item()**2 for p in m.parameters() if p.grad is not None)
        norms[n] = t**0.5
    return norms
...
def generate_text(model, tokenizer, seed="The", length=60):
    print(f"\n── Generating ('{seed}') ──")
    model.eval()
    ...
    with torch.no_grad():
        try:
            ...
            for _ in range(length):
                logits, _, mem, _, _, _ = model(curr, mem)
...
def calc_loss(logits, y, stats, meta, plast, ep, tot_ep):
    ...
    if isinstance(d, torch.Tensor):
        d_pen = CONFIG['depth_target_weight'] * ((d - CONFIG['target_depth'])**2).mean()
    else: d_pen = 0
...
def visualize_suite(model, history, transitions, meta_history):
    # [FIX] Simplified Diagnostics to prevent matplotlib memory leaks/crashes
    if not history.get('loss'): return
    print("\n--- Saving Diagnostics ---")
    plt.figure(figsize=(12, 8))
    
    plt.subplot(2,2,1); plt.plot(history['loss'], label='Loss'); plt.legend()
    plt.subplot(2,2,2); plt.plot(history['depth'], color='purple', label='Depth'); plt.legend()
...
def main():
    ...
    generate_text(model, tok)
    visualize_suite(model, hist, transitions, meta_hist)
