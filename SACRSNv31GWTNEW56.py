"""
╔══════════════════════════════════════════════════════════════════════════╗
║         SACRSN — UNIFIED EDITION  (v79 Algebraic Stability)            ║
...
print(f"--- SACRSN v79: ALGEBRAIC STABILITY EDITION ---")
...
class ComplexLayerNorm(nn.Module):
    ...
    [FIX] Replaced z.angle() with z / |z| to avoid NaN gradients at zero.
    """
    ...
    def forward(self, z):
        ...
        # Normalize magnitude (Real math)
        mean = mag.mean(dim=-1, keepdim=True)
        var = mag.var(dim=-1, keepdim=True)
        norm_mag = (mag - mean) / (var + eps).sqrt() * self.scale + self.shift
        
        # Restore phase algebraically: z_unit = z / (|z| + eps)
        # This replaces: angle = z.angle(); cos(angle), sin(angle)
        z_unit = z / (mag.unsqueeze(-1) + 1e-9) # Broadcast safe div
        
        # Combine: norm_mag * z_unit. 
        # norm_mag is real, z_unit is complex.
...
class SACRSN_Unified(nn.Module):
    ...
    def forward_step(self, x_t, gw_state, mem_state):
        ...
        # [FIX] Added epsilon 1e-9 to prevent angle() backward error at start
        ...
        for i in range(CONFIG["max_recursion"]):
            ...
            # [FIX] Safer Phase Reg Calculation with epsilon jitter
            cand_safe = cand + 1e-9
            mag = cand_safe.abs()
            valid = mag > 1e-6
            safe_angle = torch.where(valid, cand_safe.angle(), prev_angle)
